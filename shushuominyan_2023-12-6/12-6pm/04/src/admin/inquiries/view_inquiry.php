...
if(isset($_GET['id']) && $_GET['id'] > 0){  
    $stmt = $conn->prepare("SELECT * from `inquiry_list` where `id` = :id");  
...
k = $v;  
        ...
        $stmt = $conn->prepare("UPDATE `inquiry_list` set `status` = 1 where `id` = :id");  
        $stmt->bind_param("i", $_GET['id']);  
        $stmt->execute();  
    } else {  
    ...
    <div class="col-lg-10 col-md-12 col-sm-12 col-xs-12">
        <div class="card rounded-0">
            ...
            <div class="card-footer py-1 text-center">
                <button class="btn btn-danger btn-sm bg-gradient-danger rounded-0" type="button" id="delete_data"><i
                        class="fa fa-trash"></i> Delete</button>
                <a class="btn btn-light btn-sm bg-gradient-light border rounded-0" href="./?page=inquiries"><i
...
$(function() {
    $('#delete_data').click(function() {
        _conf("Are you sure to delete this inquiry permanently?", "delete_inquiry", [
            "<?= isset($id) ? $id :'' ?>"
        ])
    })
})
